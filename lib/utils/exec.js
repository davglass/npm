...
function exec (cmd, args, env, takeOver, cwd, uid, gid, cb) {
  ...
  gid = gid == null ? npm.config.get("group") : gid
  uid = uid == null ? npm.config.get("operator") : uid
  log.silly(cmd+" "+args.map(JSON.stringify).join(" "), "exec")
  var stdout = ""
    , stderr = ""
    , cp = spawn(cmd, args, env, takeOver, uid, gid, cwd)
...
function spawn (c, a, env, takeOver, uid, gid, cwd) {
  ...
  log.silly([c, a, opts.uid, opts.gid], "spawn")
