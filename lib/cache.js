      ...
      sha.get(p, function (er, shasum) {
        ...
        readJson(path.join(folder, "package.json"), function (er, data) {
          ...
          fs.writeFile(json, JSON.stringify(data,null,2), function (er) {
            ...
            var uid = npm.config.get("operator")
              , gid = npm.config.get("group")
...
function unpack (pkg, ver, unpackTarget, uid, gid, cb) {
...
function unpackTar (tarball, unpackTarget, uid, gid, cb) {
  if (!cb) cb = gid, gid = null
  if (!cb) cb = uid, uid = null
  if (uid == null) uid = npm.config.get("operator")
  if (gid == null) gid = npm.config.get("group")
  if (!isNaN(uid)) uid = +uid
  if (!isNaN(gid)) gid = +gid
  log.warn([uid, gid], "unpackTar")
  mkdir(unpackTarget, uid, gid, function (er) {
    ...
    pipe( spawn( npm.config.get("gzipbin")
               ...
               , process.env, false, process.cwd(), uid, gid )
        , spawn( npm.config.get("tar")
               , ["-vxpf", "-", "-C", unpackTarget]
               , process.env, false, process.cwd(), uid, gid )
        , log.er(cb, "Failed unpacking "+tarball+" to "+unpackTarget) )
  })
}
